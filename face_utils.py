...
def detect_faces(image):
    ...
    """
    if not FACE_RECOGNITION_AVAILABLE or cv2 is None or np is None:
        return []
...
def compare_faces(known_encoding, new_encoding, tolerance=0.6):
    ...
    """
    if known_encoding is None or new_encoding is None or not FACE_RECOGNITION_AVAILABLE:
...
class FaceUtils:
    ...
    def register_face(self, email, image_data):
        ...
        if not FACE_RECOGNITION_AVAILABLE:
            return False, "Face recognition support is unavailable"
    ...
    def verify_face(self, email, image_data, tolerance=0.6):
        ...
        """
        if not FACE_RECOGNITION_AVAILABLE:
            return False, "Face recognition support is unavailable", float('inf')
